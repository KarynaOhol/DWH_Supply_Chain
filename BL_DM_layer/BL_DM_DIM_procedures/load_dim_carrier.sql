...
-- Purpose: Load CARRIERS dimension from BL_3NF to BL_DM
...
CREATE OR REPLACE FUNCTION BL_CL.get_carrier_column_mapping(
...
BEGIN
    ...
    CASE p_source_system
        WHEN '3NF_LAYER' THEN v_column_mapping :=
                'carrier_src_id, carrier_name, carrier_type, ''3NF_LAYER'' as source_system, ''CE_CARRIERS'' as source_entity, ta_update_dt';
...
CREATE OR REPLACE FUNCTION BL_CL.build_carrier_where_clause(
...
$$
DECLARE
    v_where_clause TEXT := 'WHERE carrier_id != -1';
BEGIN
...
-- Function to validate carrier data quality
CREATE OR REPLACE FUNCTION BL_CL.validate_carrier_data(
    p_config BL_CL.t_dim_load_config
...
BEGIN
    ...
    EXECUTE FORMAT('SELECT COUNT(*) FROM %s WHERE carrier_id != -1', p_config.source_table)
        INTO v_total_count;

    -- Validation 1: Missing or Invalid Carrier Names
    v_sql := FORMAT(
            'SELECT COUNT(*), STRING_AGG(DISTINCT carrier_src_id::TEXT, '', '') FROM %s WHERE carrier_id != -1 AND (carrier_name IS NULL OR carrier_name = '''' OR carrier_name = ''Unknown'')',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Missing or Invalid Carrier Names',
        v_failed_count,
        v_total_count,
        ...
        CASE
            WHEN v_failed_count > 0 THEN 'Review carrier name data quality'
            ELSE 'Carrier name validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);

    -- Validation 2: Missing Carrier Source IDs
    v_sql := FORMAT(
            'SELECT COUNT(*), STRING_AGG(DISTINCT carrier_id::TEXT, '', '') FROM %s WHERE carrier_id != -1 AND (carrier_src_id IS NULL OR carrier_src_id = '''')',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Missing Carrier Source IDs',
        v_failed_count,
        v_total_count,
        ROUND((v_failed_count::DECIMAL / NULLIF(v_total_count, 0)) * 100, 2),
        CASE
        ...
        COALESCE(v_sample_records, 'None'),
        CASE
            WHEN v_failed_count > 0 THEN 'Critical: Carrier source IDs are required for dimension'
            ELSE 'Carrier source ID validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);

    -- Validation 3: Duplicate Carrier Source IDs
    v_sql := FORMAT(
            'SELECT COUNT(*) - COUNT(DISTINCT carrier_src_id), STRING_AGG(DISTINCT carrier_src_id::TEXT, '', '') FROM (SELECT carrier_src_id FROM %s WHERE carrier_id != -1 GROUP BY carrier_src_id HAVING COUNT(*) > 1) dups',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Duplicate Carrier Source IDs',
        v_failed_count,
        v_total_count,
        ROUND((v_failed_count::DECIMAL / NULLIF(v_total_count, 0)) * 100, 2),
        CASE
        ...
        COALESCE(v_sample_records, 'None'),
        CASE
            WHEN v_failed_count > 0 THEN 'Review duplicate carrier source IDs - may cause UPSERT issues'
            ELSE 'Carrier uniqueness validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);

    -- Validation 4: Invalid Carrier Types
    v_sql := FORMAT(
            'SELECT COUNT(*), STRING_AGG(DISTINCT carrier_src_id::TEXT, '', '') FROM %s WHERE carrier_id != -1 AND (carrier_type IS NOT NULL AND carrier_type NOT IN (''Standard'', ''Express'', ''Freight'', ''Air'', ''Ground'', ''Ocean''))',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Invalid Carrier Types',
        v_failed_count,
        v_total_count,
        ROUND((v_failed_count::DECIMAL / NULLIF(v_total_count, 0)) * 100, 2),
        CASE
        ...
        COALESCE(v_sample_records, 'None'),
        CASE
            WHEN v_failed_count > 0 THEN 'Review carrier type values - may need standardization'
            ELSE 'Carrier type validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);
...
-- MAIN PROCEDURE: Load DIM_CARRIERS using composite types, dynamic SQL, and UPSERT
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_carriers(
...
BEGIN
    ...
    IF NOT BL_CL.acquire_procedure_lock('load_dim_carriers') THEN
        RAISE EXCEPTION 'Procedure load_dim_carriers is already running';
    END IF;

    -- Initialize configuration with defaults if not provided
    v_config := COALESCE(p_config, ROW (
        'BL_3NF.CE_CARRIERS', -- source_table
        'BL_DM.DIM_CARRIERS', -- target_table
        'CARRIER_SRC_ID', -- business_key_column
        'CARRIER_SURR_ID', -- surrogate_key_column
        'DELTA', -- load_mode
    ...
    IF v_config.enable_logging THEN
        CALL BL_CL.log_procedure_event(
                ...
                'START',
                0,
                FORMAT('Starting DIM_CARRIERS load - Mode: %s, Source System: %s, Validation: %s',
    ...
    IF v_config.validation_level IN ('STRICT', 'RELAXED') THEN
        v_validations := BL_CL.validate_carrier_data(v_config);
        ...
        FOR i IN 1..array_length(v_validations, 1)
            LOOP
                ...
                IF v_validation.severity = 'ERROR' AND v_config.validation_level = 'STRICT' THEN
                    ...
                    IF v_config.enable_logging THEN
                        CALL BL_CL.log_procedure_event(
                                'load_dim_carriers', v_source_table, v_target_table, 'ERROR',
                ...
                ELSIF v_validation.severity IN ('ERROR', 'WARNING') AND v_config.enable_logging THEN
                    CALL BL_CL.log_procedure_event(
                            'load_dim_carriers', v_source_table, v_target_table, 'WARNING',
    ...
    IF v_config.load_mode = 'DELTA' THEN
        v_last_update_dt := BL_CL.get_last_successful_load('load_dim_carriers');
    END IF;

    -- STEP 3: Build dynamic SQL components
    v_column_mapping := BL_CL.get_carrier_column_mapping(v_config.include_source_system);
    v_where_clause := BL_CL.build_carrier_where_clause(v_config, v_last_update_dt);
    ...
    v_sql := FORMAT('
        ...
        upsert_result AS (
            INSERT INTO %s (
                %s, CARRIER_NAME, CARRIER_TYPE, SOURCE_SYSTEM, SOURCE_ENTITY,
                TA_INSERT_DT, TA_UPDATE_DT
            )
            SELECT
                carrier_src_id,
                COALESCE(carrier_name, ''Unknown''),
                COALESCE(carrier_type, ''Standard''),
                source_system,
                source_entity,
                CURRENT_TIMESTAMP,
                CURRENT_TIMESTAMP
            ...
            WHERE (
                %s.CARRIER_NAME != EXCLUDED.CARRIER_NAME OR
                %s.CARRIER_TYPE != EXCLUDED.CARRIER_TYPE
            )
            RETURNING
        ...
        FROM upsert_result',
                    ...
                    v_target_table, v_target_table -- Table aliases for UPDATE condition
             );
    ...
    v_result := ROW (
        ...
        FORMAT('DIM_CARRIERS load completed - Mode: %s, Inserted: %s, Updated: %s, Unchanged: %s',
    ...
    PERFORM BL_CL.release_procedure_lock('load_dim_carriers');
...
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_carriers_delta()
    LANGUAGE plpgsql
AS
$$
BEGIN
    CALL BL_CL.load_dim_carriers(
            ROW (
                'BL_3NF.CE_CARRIERS', 'BL_DM.DIM_CARRIERS',
                'CARRIER_SRC_ID', 'CARRIER_SURR_ID',
...
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_carriers_full()
    LANGUAGE plpgsql
AS
$$
BEGIN
...
WHERE routine_schema = 'bl_cl'
  AND routine_name LIKE '%carrier%'
ORDER BY routine_name;

-- Test the utility functions
SELECT BL_CL.get_carrier_column_mapping('3NF_LAYER') as column_mapping_test;

SELECT BL_CL.build_carrier_where_clause(
               ROW ('BL_3NF.CE_CARRIERS', 'BL_DM.DIM_CARRIERS', 'CARRIER_SRC_ID', 'CARRIER_SURR_ID',
...
-- READY FOR TESTING
-- Usage Examples:
-- CALL bl_cl.load_dim_carriers_full()
-- -- 1. Default delta load:
-- -- CALL BL_CL.load_dim_carriers_delta();
-- --
-- -- 2. Full reload:
-- -- CALL BL_CL.load_dim_carriers_full();
-- --
-- -- 3. Check results:
-- SELECT COUNT(*) FROM BL_DM.DIM_CARRIERS;
-- SELECT * FROM BL_DM.DIM_CARRIERS LIMIT 10;
-- =====================================================