...
CREATE OR REPLACE FUNCTION BL_CL.get_shipping_mode_column_mapping(
...
BEGIN
    ...
    CASE p_source_system
        WHEN '3NF_LAYER' THEN v_column_mapping :=
                'shipping_mode_src_id, shipping_mode, ''3NF_LAYER'' as source_system, ''CE_SHIPPING_MODES'' as source_entity, ta_update_dt';
...
CREATE OR REPLACE FUNCTION BL_CL.build_shipping_mode_where_clause(
...
$$
DECLARE
    v_where_clause TEXT := 'WHERE shipping_mode_id != -1';
BEGIN
...
-- Function to validate shipping mode data quality
CREATE OR REPLACE FUNCTION BL_CL.validate_shipping_mode_data(
    p_config BL_CL.t_dim_load_config
...
BEGIN
    ...
    EXECUTE FORMAT('SELECT COUNT(*) FROM %s WHERE shipping_mode_id != -1', p_config.source_table)
        INTO v_total_count;

    -- Validation 1: Missing or Invalid Shipping Mode Values
    v_sql := FORMAT(
            'SELECT COUNT(*), STRING_AGG(DISTINCT shipping_mode_src_id::TEXT, '', '') FROM %s WHERE shipping_mode_id != -1 AND (shipping_mode IS NULL OR shipping_mode = '''' OR shipping_mode = ''Unknown'')',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Missing or Invalid Shipping Mode Values',
        v_failed_count,
        v_total_count,
        ...
        CASE
            WHEN v_failed_count > 0 THEN 'Review shipping mode value data quality'
            ELSE 'Shipping mode value validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);

    -- Validation 2: Missing Shipping Mode Source IDs
    v_sql := FORMAT(
            'SELECT COUNT(*), STRING_AGG(DISTINCT shipping_mode_id::TEXT, '', '') FROM %s WHERE shipping_mode_id != -1 AND (shipping_mode_src_id IS NULL OR shipping_mode_src_id = '''')',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Missing Shipping Mode Source IDs',
        v_failed_count,
        v_total_count,
        ROUND((v_failed_count::DECIMAL / NULLIF(v_total_count, 0)) * 100, 2),
        CASE
        ...
        COALESCE(v_sample_records, 'None'),
        CASE
            WHEN v_failed_count > 0 THEN 'Critical: Shipping mode source IDs are required for dimension'
            ELSE 'Shipping mode source ID validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);

    -- Validation 3: Duplicate Shipping Mode Source IDs
    v_sql := FORMAT(
            'SELECT COUNT(*) - COUNT(DISTINCT shipping_mode_src_id), STRING_AGG(DISTINCT shipping_mode_src_id::TEXT, '', '') FROM (SELECT shipping_mode_src_id FROM %s WHERE shipping_mode_id != -1 GROUP BY shipping_mode_src_id HAVING COUNT(*) > 1) dups',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Duplicate Shipping Mode Source IDs',
        v_failed_count,
        v_total_count,
        ROUND((v_failed_count::DECIMAL / NULLIF(v_total_count, 0)) * 100, 2),
        CASE
        ...
        COALESCE(v_sample_records, 'None'),
        CASE
            WHEN v_failed_count > 0 THEN 'Review duplicate shipping mode source IDs - may cause UPSERT issues'
            ELSE 'Shipping mode uniqueness validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);

    -- Validation 4: Standard Shipping Mode Values Check
    v_sql := FORMAT(
            'SELECT COUNT(*), STRING_AGG(DISTINCT shipping_mode_src_id::TEXT, '', '') FROM %s WHERE shipping_mode_id != -1 AND (shipping_mode IS NOT NULL AND UPPER(shipping_mode) NOT IN (''STANDARD'', ''EXPRESS'', ''OVERNIGHT'', ''TWO-DAY'', ''GROUND'', ''AIR'', ''FREIGHT'', ''SAME DAY'', ''NEXT DAY'', ''ECONOMY'', ''PRIORITY'', ''EXPEDITED''))',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Non-Standard Shipping Mode Values',
        v_failed_count,
        v_total_count,
        ROUND((v_failed_count::DECIMAL / NULLIF(v_total_count, 0)) * 100, 2),
        CASE
        ...
        COALESCE(v_sample_records, 'None'),
        CASE
            WHEN v_failed_count > 0 THEN 'Review non-standard shipping mode values - may need standardization'
            ELSE 'Shipping mode standardization validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);
...
-- MAIN PROCEDURE: Load DIM_SHIPPING_MODES using composite types, dynamic SQL, and UPSERT
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_shipping_modes(
...
BEGIN
    ...
    IF NOT BL_CL.acquire_procedure_lock('load_dim_shipping_modes') THEN
        RAISE EXCEPTION 'Procedure load_dim_shipping_modes is already running';
    END IF;

    -- Initialize configuration with defaults if not provided
    v_config := COALESCE(p_config, ROW (
        'BL_3NF.CE_SHIPPING_MODES', -- source_table
        'BL_DM.DIM_SHIPPING_MODES', -- target_table
        'SHIPPING_MODE_SRC_ID', -- business_key_column
        'SHIPPING_MODE_SURR_ID', -- surrogate_key_column
        'DELTA', -- load_mode
    ...
    IF v_config.enable_logging THEN
        CALL BL_CL.log_procedure_event(
                ...
                'START',
                0,
                FORMAT('Starting DIM_SHIPPING_MODES load - Mode: %s, Source System: %s, Validation: %s',
    ...
    IF v_config.validation_level IN ('STRICT', 'RELAXED') THEN
        v_validations := BL_CL.validate_shipping_mode_data(v_config);
        ...
        FOR i IN 1..array_length(v_validations, 1)
            LOOP
                ...
                IF v_validation.severity = 'ERROR' AND v_config.validation_level = 'STRICT' THEN
                    ...
                    IF v_config.enable_logging THEN
                        CALL BL_CL.log_procedure_event(
                                'load_dim_shipping_modes', v_source_table, v_target_table, 'ERROR',
                ...
                ELSIF v_validation.severity IN ('ERROR', 'WARNING') AND v_config.enable_logging THEN
                    CALL BL_CL.log_procedure_event(
                            'load_dim_shipping_modes', v_source_table, v_target_table, 'WARNING',
    ...
    IF v_config.load_mode = 'DELTA' THEN
        v_last_update_dt := BL_CL.get_last_successful_load('load_dim_shipping_modes');
    END IF;

    -- STEP 3: Build dynamic SQL components
    v_column_mapping := BL_CL.get_shipping_mode_column_mapping(v_config.include_source_system);
    v_where_clause := BL_CL.build_shipping_mode_where_clause(v_config, v_last_update_dt);
    ...
    v_sql := FORMAT('
        ...
        upsert_result AS (
            INSERT INTO %s (
                %s, SHIPPING_MODE, SOURCE_SYSTEM, SOURCE_ENTITY,
                TA_INSERT_DT, TA_UPDATE_DT
            )
            SELECT
                shipping_mode_src_id,
                COALESCE(shipping_mode, ''Standard''),
                source_system,
                source_entity,
                CURRENT_TIMESTAMP,
                CURRENT_TIMESTAMP
            ...
            WHERE (
                %s.SHIPPING_MODE != EXCLUDED.SHIPPING_MODE
            )
            RETURNING
    ...
    v_result := ROW (
        ...
        FORMAT('DIM_SHIPPING_MODES load completed - Mode: %s, Inserted: %s, Updated: %s, Unchanged: %s',
    ...
    PERFORM BL_CL.release_procedure_lock('load_dim_shipping_modes');
...
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_shipping_modes_delta()
    LANGUAGE plpgsql
AS
$$
BEGIN
    CALL BL_CL.load_dim_shipping_modes(
            ROW (
                'BL_3NF.CE_SHIPPING_MODES', 'BL_DM.DIM_SHIPPING_MODES',
                'SHIPPING_MODE_SRC_ID', 'SHIPPING_MODE_SURR_ID',
...
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_shipping_modes_full()
    LANGUAGE plpgsql
AS
$$
BEGIN
...
WHERE routine_schema = 'bl_cl'
  AND routine_name LIKE '%shipping_mode%'
ORDER BY routine_name;

-- Test the utility functions
SELECT BL_CL.get_shipping_mode_column_mapping('3NF_LAYER') as column_mapping_test;

SELECT BL_CL.build_shipping_mode_where_clause(
               ROW ('BL_3NF.CE_SHIPPING_MODES', 'BL_DM.DIM_SHIPPING_MODES', 'SHIPPING_MODE_SRC_ID', 'SHIPPING_MODE_SURR_ID',
...
-- -- CALL BL_CL.load_dim_shipping_modes_delta();
-- --
-- -- 2. Full reload:
-- -- CALL BL_CL.load_dim_shipping_modes_full();
-- --
-- -- 3. Check results:
-- SELECT COUNT(*) FROM BL_DM.DIM_SHIPPING_MODES;
-- SELECT * FROM BL_DM.DIM_SHIPPING_MODES LIMIT 10;
-- =====================================================