...
-- DIM_DELIVERY_STATUSES Loading Procedure
...
CREATE OR REPLACE FUNCTION BL_CL.get_delivery_status_column_mapping(
...
BEGIN
    ...
    CASE p_source_system
        WHEN '3NF_LAYER' THEN v_column_mapping :=
                'delivery_status_src_id, delivery_status, ''3NF_LAYER'' as source_system, ''CE_DELIVERY_STATUSES'' as source_entity, ta_update_dt';
...
CREATE OR REPLACE FUNCTION BL_CL.build_delivery_status_where_clause(
...
$$
DECLARE
    v_where_clause TEXT := 'WHERE delivery_status_id != -1';
BEGIN
...
-- Function to validate delivery status data quality
CREATE OR REPLACE FUNCTION BL_CL.validate_delivery_status_data(
    p_config BL_CL.t_dim_load_config
...
BEGIN
    ...
    EXECUTE FORMAT('SELECT COUNT(*) FROM %s WHERE delivery_status_id != -1', p_config.source_table)
        INTO v_total_count;

    -- Validation 1: Missing or Invalid Delivery Status Values
    v_sql := FORMAT(
            'SELECT COUNT(*), STRING_AGG(DISTINCT delivery_status_src_id::TEXT, '', '') FROM %s WHERE delivery_status_id != -1 AND (delivery_status IS NULL OR delivery_status = '''' OR delivery_status = ''Unknown'')',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Missing or Invalid Delivery Status Values',
        v_failed_count,
        v_total_count,
        ...
        CASE
            WHEN v_failed_count > 0 THEN 'Review delivery status value data quality'
            ELSE 'Delivery status value validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);

    -- Validation 2: Missing Delivery Status Source IDs
    v_sql := FORMAT(
            'SELECT COUNT(*), STRING_AGG(DISTINCT delivery_status_id::TEXT, '', '') FROM %s WHERE delivery_status_id != -1 AND (delivery_status_src_id IS NULL OR delivery_status_src_id = '''')',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Missing Delivery Status Source IDs',
        v_failed_count,
        v_total_count,
        ROUND((v_failed_count::DECIMAL / NULLIF(v_total_count, 0)) * 100, 2),
        CASE
        ...
        COALESCE(v_sample_records, 'None'),
        CASE
            WHEN v_failed_count > 0 THEN 'Critical: Delivery status source IDs are required for dimension'
            ELSE 'Delivery status source ID validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);

    -- Validation 3: Duplicate Delivery Status Source IDs
    v_sql := FORMAT(
            'SELECT COUNT(*) - COUNT(DISTINCT delivery_status_src_id), STRING_AGG(DISTINCT delivery_status_src_id::TEXT, '', '') FROM (SELECT delivery_status_src_id FROM %s WHERE delivery_status_id != -1 GROUP BY delivery_status_src_id HAVING COUNT(*) > 1) dups',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Duplicate Delivery Status Source IDs',
        v_failed_count,
        v_total_count,
        ROUND((v_failed_count::DECIMAL / NULLIF(v_total_count, 0)) * 100, 2),
        CASE
        ...
        COALESCE(v_sample_records, 'None'),
        CASE
            WHEN v_failed_count > 0 THEN 'Review duplicate delivery status source IDs - may cause UPSERT issues'
            ELSE 'Delivery status uniqueness validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);

    -- Validation 4: Standard Delivery Status Values Check
    v_sql := FORMAT(
            'SELECT COUNT(*), STRING_AGG(DISTINCT delivery_status_src_id::TEXT, '', '') FROM %s WHERE delivery_status_id != -1 AND (delivery_status IS NOT NULL AND UPPER(delivery_status) NOT IN (''DELIVERED'', ''IN_TRANSIT'', ''PENDING'', ''CANCELLED'', ''FAILED'', ''RETURNED'', ''DELAYED'', ''OUT_FOR_DELIVERY'', ''ATTEMPTED'', ''EXCEPTION'', ''PROCESSING'', ''SHIPPED''))',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Non-Standard Delivery Status Values',
        v_failed_count,
        v_total_count,
        ROUND((v_failed_count::DECIMAL / NULLIF(v_total_count, 0)) * 100, 2),
        CASE
        ...
        COALESCE(v_sample_records, 'None'),
        CASE
            WHEN v_failed_count > 0 THEN 'Review non-standard delivery status values - may need standardization'
            ELSE 'Delivery status standardization validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);
...
-- MAIN PROCEDURE: Load DIM_DELIVERY_STATUSES using composite types, dynamic SQL, and UPSERT
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_delivery_statuses(
...
BEGIN
    ...
    IF NOT BL_CL.acquire_procedure_lock('load_dim_delivery_statuses') THEN
        RAISE EXCEPTION 'Procedure load_dim_delivery_statuses is already running';
    END IF;

    -- Initialize configuration with defaults if not provided
    v_config := COALESCE(p_config, ROW (
        'BL_3NF.CE_DELIVERY_STATUSES', -- source_table
        'BL_DM.DIM_DELIVERY_STATUSES', -- target_table
        'DELIVERY_STATUS_SRC_ID', -- business_key_column
        'DELIVERY_STATUS_SURR_ID', -- surrogate_key_column
        'DELTA', -- load_mode
    ...
    IF v_config.enable_logging THEN
        CALL BL_CL.log_procedure_event(
                ...
                'START',
                0,
                FORMAT('Starting DIM_DELIVERY_STATUSES load - Mode: %s, Source System: %s, Validation: %s',
    ...
    IF v_config.validation_level IN ('STRICT', 'RELAXED') THEN
        v_validations := BL_CL.validate_delivery_status_data(v_config);
        ...
        FOR i IN 1..array_length(v_validations, 1)
            LOOP
                ...
                IF v_validation.severity = 'ERROR' AND v_config.validation_level = 'STRICT' THEN
                    ...
                    IF v_config.enable_logging THEN
                        CALL BL_CL.log_procedure_event(
                                'load_dim_delivery_statuses', v_source_table, v_target_table, 'ERROR',
                ...
                ELSIF v_validation.severity IN ('ERROR', 'WARNING') AND v_config.enable_logging THEN
                    CALL BL_CL.log_procedure_event(
                            'load_dim_delivery_statuses', v_source_table, v_target_table, 'WARNING',
    ...
    IF v_config.load_mode = 'DELTA' THEN
        v_last_update_dt := BL_CL.get_last_successful_load('load_dim_delivery_statuses');
    END IF;

    -- STEP 3: Build dynamic SQL components
    v_column_mapping := BL_CL.get_delivery_status_column_mapping(v_config.include_source_system);
    v_where_clause := BL_CL.build_delivery_status_where_clause(v_config, v_last_update_dt);
    ...
    v_sql := FORMAT('
        ...
        upsert_result AS (
            INSERT INTO %s (
                %s, DELIVERY_STATUS, SOURCE_SYSTEM, SOURCE_ENTITY,
                TA_INSERT_DT, TA_UPDATE_DT
            )
            SELECT
                delivery_status_src_id,
            ...
            WHERE (
                %s.DELIVERY_STATUS != EXCLUDED.DELIVERY_STATUS
            )
            RETURNING
    ...
    v_result := ROW (
        ...
        FORMAT('DIM_DELIVERY_STATUSES load completed - Mode: %s, Inserted: %s, Updated: %s, Unchanged: %s',
    ...
    PERFORM BL_CL.release_procedure_lock('load_dim_delivery_statuses');
...
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_delivery_statuses_delta()
    LANGUAGE plpgsql
AS
$$
BEGIN
    CALL BL_CL.load_dim_delivery_statuses(
            ROW (
                'BL_3NF.CE_DELIVERY_STATUSES', 'BL_DM.DIM_DELIVERY_STATUSES',
                'DELIVERY_STATUS_SRC_ID', 'DELIVERY_STATUS_SURR_ID',
...
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_delivery_statuses_full()
    LANGUAGE plpgsql
AS
$$
BEGIN
...
WHERE routine_schema = 'bl_cl'
  AND routine_name LIKE '%delivery_status%'
ORDER BY routine_name;

-- Test the utility functions
SELECT BL_CL.get_delivery_status_column_mapping('3NF_LAYER') as column_mapping_test;

SELECT BL_CL.build_delivery_status_where_clause(
               ROW ('BL_3NF.CE_DELIVERY_STATUSES', 'BL_DM.DIM_DELIVERY_STATUSES', 'DELIVERY_STATUS_SRC_ID', 'DELIVERY_STATUS_SURR_ID',
...
-- CALL BL_CL.load_dim_delivery_statuses_delta();
--
-- 2. Full reload:
-- CALL BL_CL.load_dim_delivery_statuses_full();
-- --
-- -- 3. Check results:
-- SELECT COUNT(*) FROM BL_DM.DIM_DELIVERY_STATUSES;
-- SELECT * FROM BL_DM.DIM_DELIVERY_STATUSES LIMIT 10;
-- =====================================================