...
-- File: 02_Dimension_Procedures/Business_Entities/load_dim_sales_representatives.sql
-- Purpose: Load SALES_REPRESENTATIVES dimension from BL_3NF to BL_DM
...
-- Pattern: Follows corrected business key approach from geography procedure
...
CREATE OR REPLACE FUNCTION BL_CL.get_sales_rep_column_mapping(
...
BEGIN
    ...
    CASE p_source_system
        WHEN '3NF_LAYER' THEN v_column_mapping :=
                'sales_rep_src_id, sales_rep_name, ''3NF_LAYER'' as source_system, ''CE_SALES_REPRESENTATIVES'' as source_entity, ta_update_dt';
...
CREATE OR REPLACE FUNCTION BL_CL.build_sales_rep_where_clause(
...
$$
DECLARE
    v_where_clause TEXT := 'WHERE sales_rep_id != -1';
BEGIN
...
-- Function to validate sales representative data quality
CREATE OR REPLACE FUNCTION BL_CL.validate_sales_rep_data(
    p_config BL_CL.t_dim_load_config
...
BEGIN
    ...
    EXECUTE FORMAT('SELECT COUNT(*) FROM %s WHERE sales_rep_id != -1', p_config.source_table)
        INTO v_total_count;

    -- Validation 1: Missing or Invalid Sales Rep Names
    v_sql := FORMAT(
            'SELECT COUNT(*), STRING_AGG(DISTINCT sales_rep_src_id::TEXT, '', '') FROM %s WHERE sales_rep_id != -1 AND (sales_rep_name IS NULL OR sales_rep_name = '''' OR sales_rep_name = ''Unknown'')',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Missing or Invalid Sales Rep Names',
        v_failed_count,
        v_total_count,
        ...
        CASE
            WHEN v_failed_count > 0 THEN 'Review sales representative name data quality'
            ELSE 'Sales rep name validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);

    -- Validation 2: Missing or Invalid Sales Rep Source IDs
    v_sql := FORMAT(
            'SELECT COUNT(*), STRING_AGG(DISTINCT sales_rep_id::TEXT, '', '') FROM %s WHERE sales_rep_id != -1 AND (sales_rep_src_id IS NULL OR sales_rep_src_id = '''')',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Missing Sales Rep Source IDs',
        v_failed_count,
        v_total_count,
        ROUND((v_failed_count::DECIMAL / NULLIF(v_total_count, 0)) * 100, 2),
        CASE
        ...
        COALESCE(v_sample_records, 'None'),
        CASE
            WHEN v_failed_count > 0 THEN 'Critical: Sales rep source IDs are required for dimension'
            ELSE 'Sales rep source ID validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);

    -- Validation 3: Duplicate Sales Rep Source IDs
    v_sql := FORMAT(
            'SELECT COUNT(*) - COUNT(DISTINCT sales_rep_src_id), STRING_AGG(DISTINCT sales_rep_src_id::TEXT, '', '') FROM (SELECT sales_rep_src_id FROM %s WHERE sales_rep_id != -1 GROUP BY sales_rep_src_id HAVING COUNT(*) > 1) dups',
            p_config.source_table);
    EXECUTE v_sql INTO v_failed_count, v_sample_records;

    v_validation := ROW (
        'Duplicate Sales Rep Source IDs',
        v_failed_count,
        v_total_count,
        ROUND((v_failed_count::DECIMAL / NULLIF(v_total_count, 0)) * 100, 2),
        CASE
        ...
        COALESCE(v_sample_records, 'None'),
        CASE
            WHEN v_failed_count > 0 THEN 'Review duplicate sales rep source IDs - may cause UPSERT issues'
            ELSE 'Sales rep uniqueness validation passed' END
        )::BL_CL.t_dim_validation_result;
    v_validations := array_append(v_validations, v_validation);
...
-- MAIN PROCEDURE: Load DIM_SALES_REPRESENTATIVES using composite types, dynamic SQL, and UPSERT
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_sales_representatives(
...
BEGIN
    ...
    IF NOT BL_CL.acquire_procedure_lock('load_dim_sales_representatives') THEN
        RAISE EXCEPTION 'Procedure load_dim_sales_representatives is already running';
    END IF;

    -- Initialize configuration with defaults if not provided
    v_config := COALESCE(p_config, ROW (
        'BL_3NF.CE_SALES_REPRESENTATIVES', -- source_table
        'BL_DM.DIM_SALES_REPRESENTATIVES', -- target_table
        'SALES_REP_SRC_ID', -- business_key_column
        'SALES_REP_SURR_ID', -- surrogate_key_column
        'DELTA', -- load_mode
    ...
    IF v_config.enable_logging THEN
        CALL BL_CL.log_procedure_event(
                'load_dim_sales_representatives',
                v_source_table,
                v_target_table,
                'START',
                0,
                FORMAT('Starting DIM_SALES_REPRESENTATIVES load - Mode: %s, Source System: %s, Validation: %s',
    ...
    IF v_config.validation_level IN ('STRICT', 'RELAXED') THEN
        v_validations := BL_CL.validate_sales_rep_data(v_config);
        ...
        FOR i IN 1..array_length(v_validations, 1)
            LOOP
                ...
                IF v_validation.severity = 'ERROR' AND v_config.validation_level = 'STRICT' THEN
                    ...
                    IF v_config.enable_logging THEN
                        CALL BL_CL.log_procedure_event(
                                'load_dim_sales_representatives', v_source_table, v_target_table, 'ERROR',
                ...
                ELSIF v_validation.severity IN ('ERROR', 'WARNING') AND v_config.enable_logging THEN
                    CALL BL_CL.log_procedure_event(
                            'load_dim_sales_representatives', v_source_table, v_target_table, 'WARNING',
    ...
    IF v_config.load_mode = 'DELTA' THEN
        v_last_update_dt := BL_CL.get_last_successful_load('load_dim_sales_representatives');
    END IF;

    -- STEP 3: Build dynamic SQL components
    v_column_mapping := BL_CL.get_sales_rep_column_mapping(v_config.include_source_system);
    v_where_clause := BL_CL.build_sales_rep_where_clause(v_config, v_last_update_dt);
    ...
    v_sql := FORMAT('
        ...
        upsert_result AS (
            INSERT INTO %s (
                %s, SALES_REP_NAME, SOURCE_SYSTEM, SOURCE_ENTITY,
                TA_INSERT_DT, TA_UPDATE_DT
            )
            SELECT
                sales_rep_src_id,
                COALESCE(sales_rep_name, ''Unknown''),
                source_system,
                source_entity,
                CURRENT_TIMESTAMP,
                CURRENT_TIMESTAMP
            ...
            WHERE (
                %s.SALES_REP_NAME != EXCLUDED.SALES_REP_NAME
            )
            RETURNING
    ...
    v_result := ROW (
        ...
        FORMAT('DIM_SALES_REPRESENTATIVES load completed - Mode: %s, Inserted: %s, Updated: %s, Unchanged: %s',
    ...
    PERFORM BL_CL.release_procedure_lock('load_dim_sales_representatives');
...
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_sales_representatives_delta()
    LANGUAGE plpgsql
AS
$$
BEGIN
    CALL BL_CL.load_dim_sales_representatives(
            ROW (
                'BL_3NF.CE_SALES_REPRESENTATIVES', 'BL_DM.DIM_SALES_REPRESENTATIVES',
                'SALES_REP_SRC_ID', 'SALES_REP_SURR_ID',
...
CREATE OR REPLACE PROCEDURE BL_CL.load_dim_sales_representatives_full()
    LANGUAGE plpgsql
AS
$$
BEGIN
...
WHERE routine_schema = 'bl_cl'
  AND routine_name LIKE '%sales_rep%'
...
WHERE specific_schema = 'bl_cl'
  AND specific_name LIKE '%load_dim_sales_representatives%'
ORDER BY ordinal_position;

-- Test the utility functions
SELECT BL_CL.get_sales_rep_column_mapping('3NF_LAYER') as column_mapping_test;

SELECT BL_CL.build_sales_rep_where_clause(
               ROW ('BL_3NF.CE_SALES_REPRESENTATIVES', 'BL_DM.DIM_SALES_REPRESENTATIVES', 'SALES_REP_SRC_ID', 'SALES_REP_SURR_ID',
...
-- READY FOR TESTING
-- Usage Examples:
call bl_cl.load_dim_sales_representatives_full()

-- 1. Default delta load:
-- CALL BL_CL.load_dim_sales_representatives_delta();
--
-- 2. Full reload with custom config:
-- CALL BL_CL.load_dim_sales_representatives(
--     ROW('BL_3NF.CE_SALES_REPRESENTATIVES', 'BL_DM.DIM_SALES_REPRESENTATIVES',
--         'SALES_REP_SRC_ID', 'SALES_REP_SURR_ID',
--         'FULL', 'ALL', 'STRICT', 1000, TRUE)::BL_CL.t_dim_load_config
-- );
--
-- 3. Validation only:
-- SELECT * FROM BL_CL.validate_sales_rep_data(
--     ROW('BL_3NF.CE_SALES_REPRESENTATIVES', 'BL_DM.DIM_SALES_REPRESENTATIVES',
--         'SALES_REP_SRC_ID', 'SALES_REP_SURR_ID',
...
--
-- 4. Check results:
SELECT COUNT(*) FROM BL_DM.DIM_SALES_REPRESENTATIVES;
SELECT * FROM BL_DM.DIM_SALES_REPRESENTATIVES LIMIT 10;
-- =====================================================